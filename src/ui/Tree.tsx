...
export class Tree<T = {}> extends React.Component<ITreeProps<T>, {}> {
    ...
    private renderNodes(treeNodes: Array<ITreeNode<T>>, currentPath?: number[], className?: string): JSX.Element | null {
        if (treeNodes == null) {
            return null;
        }
    ...
    private handlerHelper<T>(handlerFromProps: TreeEventHandler<T> | undefined, node: TreeNode<T>, e: React.MouseEvent<HTMLElement>) {
        if (isFunction(handlerFromProps)) {
            const nodeData = Tree.nodeFromPath(node.props.path, this.props.contents) as ITreeNode<T>;
